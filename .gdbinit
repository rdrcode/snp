...
set $SHOWDATAWIN = 0
# set to 1 to enable display of bss data window (default is 0)
set $SHOWBSSWIN = 0
...
################################# X86
        ...
        if ($64BITS == 1)
            if ((($rsi >> 0x18) == 0x40) || (($rsi >> 0x18) == 0x08) || (($rsi >> 0x18) == 0xBF) || (($rsi >> 0x14) == 0x6))
                set $data_addr = $rsi
            else
                if ((($rdi >> 0x18) == 0x40) || (($rdi >> 0x18) == 0x08) || (($rdi >> 0x18) == 0xBF) || (($rdi >> 0x14) == 0x6))
                    set $data_addr = $rdi
                else
                    if ((($rax >> 0x18) == 0x40) || (($rax >> 0x18) == 0x08) || (($rax >> 0x18) == 0xBF) || (($rax >> 0x14) == 0x6))
                        set $data_addr = $rax
                    else
                        set $data_addr = $rsp
                    end
...
define bsswin
    if $ARM != 1
################################# X86
        #set $data_addr = (unsigned char *)&__bss_start
        set $data_addr = ((int)&__bss_start & 0xfffffff8) + 8
    end
    ddump $CONTEXTSIZE_BSS
end
document bsswin
Syntax: bsswin
| Display data window for bss segment
end
...
define step_to_int
    ...
    printf "step_to_int command stopped at:\n  "
    x/i $pc
    printf "\n"
end
document step_to_int
Syntax: step_to_int
| Single step until a int instruction is found.
| Stop before the int is executed.
end
...
document enablebsswin
Syntax: enablebsswin
| Enable display of bss data window in the context window.
end
...
define disablebsswin
	set $SHOWBSSWIN = 0
end
document disablebsswin
Syntax: disablebsswin
| Disable display of bss data window in the context window.
end
